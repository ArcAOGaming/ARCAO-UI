        ...
        >
          ...
          <div style={{ 
            ...
            <div style={{ marginBottom: '20px' }}>
              <h4 style={{ marginBottom: '8px', borderBottom: '1px solid #ddd', paddingBottom: '6px' }}>Process Details</h4>
              <p><strong>Process ID:</strong> <span style={{ wordBreak: 'break-all', fontSize: '0.85rem' }}>{tokenId}</span></p>
              <p><strong>Token Address:</strong> {token?.flp_token_process ? (
                <span className="token-id-truncated" title={token.flp_token_process}>
                  {`${token.flp_token_process.slice(0, 8)}...${token.flp_token_process.slice(-4)}`}
                </span>
              ) : 'N/A'}</p>
              <p><strong>Treasury:</strong> {token?.treasury ? (
                <span className="token-id-truncated" title={token.treasury}>
